...
public class UpdateSimpleOperatorTest {
    ...
    public void test1() {
        ...
        assertEquals("没有传参，且没有默认值", "update  " + TABLE + " set userId=?,userName=?  where 1=1", operator.generateSql(param).getResult());
    }
    ...
    public void test2() {
        ...
        assertEquals("没有传参，但有默认值",
                "update  " + TABLE + " set userId=?,userName=?,userSex=?,userSex2=?  where 1=1",
                model.getResult());
        assertEquals("没有传参，但有默认值  剥离参数",
                "[null,\"abc\",null,\"0\"]",
    ...
    public void test3() {
        ...
        Utils.addParam(param1);
        ResultModel model = operator.generateSql(param);
        assertEquals("有传参，有默认值",
                "update  " + TABLE + " set userId=?,userName=?,userSex=?,userSex2=?  where 1=1",
                model.getResult());
        assertEquals("有传参，有默认值  剥离参数",
                "[\"userId\",\"userName\",3,2]",
                JSONUtils.toJSONString(model.getParamsStatic()));
    }
    ...
    public void test4() {
        ...
        Utils.addParam(param1);
        ResultModel model = operator.generateSql(param);
        assertEquals("有传参，没有默认值",
                "update  " + TABLE + " set userId=?,userName=?,userName2=?,userName3=?,userSex=?,userSex2=?  where 1=1",
                model.getResult());
        assertEquals("有传参，没有默认值  剥离参数",
                "[\"userId\",null,null,null,3,2]",
                JSONUtils.toJSONString(model.getParamsStatic()));
    }
    ...
    public void test5() {
        ...
        Utils.addParam(param1);
        ResultModel model = operator.generateSql(param);
        assertEquals("老版本默认值兼容",
                "update  table set userId=?,userName=?,userSex=?,userSex2=?,userSex3=?,userSex4=NOW(),userSex5=NOW()  where 1=1",
                model.getResult());
        assertEquals("老版本默认值兼容",
                "[null,\"abc\",\"ab'c\",\"ab'c\",\"ab=c\"]",
                JSONUtils.toJSONString(model.getParamsStatic()));
    }
