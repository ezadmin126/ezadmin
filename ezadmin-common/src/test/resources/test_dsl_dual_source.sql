...
-- 1. 测试表单DSL（数据库版）
INSERT INTO T_EZADMIN_FORM (FORM_ID,
                            FORM_NAME,
                            DATASOURCE,
                            FORM_DSL,
                            INIT_EXPRESS,
...
-- 2. 测试列表DSL（数据库版）
INSERT INTO T_EZADMIN_LIST (LIST_ID,
                            LIST_NAME,
                            DATASOURCE,
                            LIST_DSL,
                            SELECT_EXPRESS,
...
-- 3. 查询验证
SELECT FORM_ID AS '表单ID', FORM_NAME AS '表单名称', DATASOURCE AS '数据源', CHAR_LENGTH(FORM_DSL) AS 'DSL长度', CHAR_LENGTH(INIT_EXPRESS) AS '初始化表达式长度', CHAR_LENGTH(SUBMIT_EXPRESS) AS '提交表达式长度'
FROM T_EZADMIN_FORM
WHERE FORM_ID = 'test-db-form';

SELECT LIST_ID AS '列表ID', LIST_NAME AS '列表名称', DATASOURCE AS '数据源', CHAR_LENGTH(LIST_DSL) AS 'DSL长度', CHAR_LENGTH(SELECT_EXPRESS) AS '查询表达式长度', CHAR_LENGTH(COUNT_EXPRESS) AS '总数表达式长度'
FROM T_EZADMIN_LIST
