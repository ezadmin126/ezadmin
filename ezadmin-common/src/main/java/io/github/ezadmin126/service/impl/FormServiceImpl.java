...
import io.github.ezadmin126.common.enums.ParamNameEnum;
...
public class FormServiceImpl implements FormService {
    ...
    public void fillFormById(Map<String, Object> form, Map<String, Object> request, Map<String, String> session) throws Exception {
        ...
        if (core != null) {
            dataSource = EzBootstrap.getInstance().getDataSourceByKey(core.get(JsoupUtil.DATASOURCE));
        } else {
            ...
            for (int i = 0; i < cardList.size(); i++) {
                ...
                if (Utils.isNotEmpty(items)) {
                    ...
                    for (int j = 0; j < items.size(); j++) {
                        ...
                        context.setVariable("uploadUrl", EzBootstrap.config().getUploadUrl());
                        ...
                        if (StringUtils.isNotBlank(dataConf)) {
                            try {
                                ...
                                if (StringUtils.isNotBlank(datasource) && EzBootstrap.getInstance().getDataSourceByKey(datasource) != null) {
                                    temp = EzBootstrap.getInstance().getDataSourceByKey(datasource);
                                }
                                Map nm = new HashMap();
    ...
    public Map<String, Object> selectConfigEditForm(String code) throws Exception {
        String sql = "select id,EZ_CODE,DATASOURCE,EZ_NAME,EZ_CONFIG from T_EZADMIN_EDIT where   EZ_CODE=? and EZ_TYPE=2 " +
                "";
        Map<String, Object> listMap = Dao.getInstance().executeQueryOne(EzBootstrap.getInstance().getEzDataSource(),
                sql, new Object[]{code});
        if (Utils.isEmpty(listMap)) {
            return listMap;
        }
