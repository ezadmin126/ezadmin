...
import io.github.ezadmin126.common.utils.JsoupUtil;
...
public class FormDao extends JsoupUtil {
    ...
    public void init() {
        for (int i = 0; i < EzBootstrap.config().getFormConfigResources().size(); i++) {
            Config item = EzBootstrap.config().getFormConfigResources().get(i);
            try {
    ...
    public Map<String, Object> selectAllFormByHtml(String ezConfig) {
        ...
        Utils.putIfAbsent(formMap, JsoupUtil.ADMINSTYLE, EzBootstrap.config().getAdminStyle());
...
//                    }
    ...
    public String transEntityToHtmlConfig(Map<String, Object> form) throws IOException {
