package io.github.ezadmin126.dao;
...
import io.github.ezadmin126.common.enums.JdbcTypeEnum;
import io.github.ezadmin126.common.enums.OperatorEnum;
...
public class ListDao extends JsoupUtil {
    ...
    private static Map<String, org.w3c.dom.Document> xmlConfigMap = new HashMap<>();
    ...
    private void loadAllLists() {
        //log.info("开始加载所有list size={}", EzBootstrap.getInstance().getListConfigResources().size());
        for (int i = 0; i < EzBootstrap.config().getListConfigResources().size(); i++) {

            Config item = EzBootstrap.config().getListConfigResources().get(i);
    ...
    void fillcore(Map<String, Object> coreMap, Document doc) {
        ...
        //adminstyle
        Utils.putIfAbsent(coreMap, JsoupUtil.ADMINSTYLE, EzBootstrap.config().getAdminStyle());
...
//        Document doc=config.getDoc();
     ...
     * 加载所有XML配置文件
     *
     * @param xmlDirPath XML文件所在目录（如 "src/main/resources/topezadmin/config/layui/list"）
     */
    public void loadAllXmlConfigs(String xmlDirPath) {
        try {
            File dir = new File(xmlDirPath);
            if (!dir.exists() || !dir.isDirectory()) {
                log.error("XML配置目录不存在: {}", xmlDirPath);
                return;
            }
            File[] files = dir.listFiles((d, name) -> name.endsWith(".xml"));
            if (files == null) {
                log.warn("目录下没有xml文件: {}", xmlDirPath);
                return;
            }
            ...
            for (File file : files) {
                ...
                xmlConfigMap.put(id.toLowerCase(), doc);
                log.info("已加载XML配置: {}", id);
            }
        } catch (Exception e) {
            log.error("加载XML配置异常", e);
        }
    }
     ...
     * 根据id获取XML配置
     */
    public org.w3c.dom.Document getXmlConfig(String id) {
        return xmlConfigMap.get(id.toLowerCase());
    }
}
