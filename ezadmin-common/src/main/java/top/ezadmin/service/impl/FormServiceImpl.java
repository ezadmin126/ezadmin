                ...
                if (Utils.isNotEmpty(items)) {
                    ...
                    for (int j = 0; j < items.size(); j++) {
                        ...
                        if (StringUtils.isNotBlank(itemUrl)) {
                            ...
                            context.put("item_url", url);
                            context.put(JsoupUtil.URL, url);
                        }
                        ...
                        if (StringUtils.isNotBlank(dataConf)) {
                            try {
                                ...
                                context.put("items", selectItems.getItems());
                                context.put("data", dataConf);
                                context.put("childItems", selectItems.getChildItems());
                                context.put("itemsJson", JSONUtils.toJSONString(selectItems.getItems()));
                                //计算checkbox显示
                            } catch (Exception e) {
                        ...
                        if (Utils.trimNull(item.get("type")).contains("xmselect")
                            ...
                            context.put("suffix", attrMap.get("suffix"));
                            context.put("serverDom", sb.toString());
                            context.put("attrMap", JSONUtils.toJSONString(attrMap));
