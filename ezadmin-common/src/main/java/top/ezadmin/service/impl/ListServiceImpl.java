...
public class ListServiceImpl implements ListService {
    ...
    ItemInitData text(String initData, Map<String, Object> request) {
        ...
        if (ShortItemValueEnum.DATABASE.name().equalsIgnoreCase(sql)) {
            ...
            for (String item : EzBootstrap.config().getDatasourceMap().keySet()) {
...
//        }
    ...
    ItemInitData kvSqlCache(final String initData, Map<String, Object> params, final DataSource datasource, String mini) {
        ...
        if ("5".equals(mini)) {
            return data.items((List<Map<String, Object>>) EzBootstrap.config().getEzCache().get("KV_SQL_CACHE_", key
                    , callback));
        }
        if ("60".equals(mini)) {
            return data.items((List<Map<String, Object>>) EzBootstrap.config().getEzCache().get60("KV_SQL_CACHE_", key
                    , callback));
        }
        return data.items((List<Map<String, Object>>) EzBootstrap.config().getEzCache().get("KV_SQL_CACHE_", key
                , callback));
    ...
    public ItemInitData getSelectItems(DataSource datasource, String initData, String initType, Map<String, Object> params
        ...
        switch (ItemDataSourceType.get(initType)) {
            ...
            case KVSQLCACHE5:
                ...
                break;
            case QLEXPRESS:
                data = qlexpress(initData, params, datasource);
                break;
            case EZLIST:
    ...
    private ItemInitData datagroup(DataSource datasource, String initData, Map<String, Object> params) throws Exception {
        ...
        Map<String, Object> group = (Map<String, Object>) EzBootstrap.config().getEzCache().get("datagroup_cache", kv[0], new Callback() {
            @Override
            public Object call(String key) {
                try {
                    Map<String, Object> m = dao.executeQueryOne(EzBootstrap.getInstance().getDataSourceByKey("defaultDataSource"),
    ...
    private ItemInitData kvTreeSqlCache(String initData, Map<String, Object> params, DataSource datasource, String s) {
        ...
        String key = StringUtils.filterLetterOrDigit(initData);
        List<Map<String, Object>> d = (List<Map<String, Object>>) EzBootstrap.config().getEzCache().get("kvtreesqlcache", key, new Callback() {
            @Override
            public Object call(String key) {
    ...
    public String selectPublishListById(String encodeId) throws Exception {
        try {
            ...
            if (!StringUtils.contains("listmanage,formmanage,datamanage", encodeId)) {
                try {
                    listMap = Dao.getInstance().executeQueryOne(EzBootstrap.getInstance().getEzDataSource(),
    ...
    public String selectDbPublishListById(String encodeId) throws Exception {
        ...
        Map<String, Object> listMap = null;
        try {
            listMap = Dao.getInstance().executeQueryOne(EzBootstrap.getInstance().getDataSourceByKey("defaultDataSource"),
                    sql, new Object[]{encodeId.toLowerCase()});
        } catch (Exception e) {
            LOG.error(encodeId, e);
        }
...
//    }
    ...
    public Map<String, Object> selectConfigEditList(String code) throws Exception {
        String sql = "select id,lower(EZ_CODE) EZ_CODE,DATASOURCE,EZ_NAME,EZ_CONFIG from T_EZADMIN_EDIT where   lower(EZ_CODE)=? and EZ_TYPE=1 " +
                "";
        Map<String, Object> listMap = Dao.getInstance().executeQueryOne(EzBootstrap.getInstance().getDataSourceByKey("defaultDataSource"),
            ...
            for (int i = 0; i < dataList.size(); i++) {
                ...
                for (int j = 0; j < colList.size(); j++) {
                    ...
                    try {
                        ...
                        context.put("_CHECK_ID_VALUE", dataRow.get("ID"));
                        ...
                        context.put(JsoupUtil.WINDOW_NAME, MapParser.parseDefaultEmpty(windowname, dataRow).getResult());
                        context.putAll(dataRow);
                        context.put("dataInDb", dataInDb);
                        context.put("uploadUrl", requestParamMap.get("ContextPath") + EzBootstrap.config().getUploadUrl());
                        if (StringUtils.startsWith(EzBootstrap.config().getDownloadUrl(), "http")) {
                            context.put("downloadUrl", EzBootstrap.config().getDownloadUrl());
                        } else {
                            context.put("downloadUrl", requestParamMap.get("ContextPath") + EzBootstrap.config().getDownloadUrl());
                        }
                        ...
                        try {
                            ...
                            if (Utils.isNotEmpty(items.getItems())) {
                                ...
                                if (containsStudent) {
                                    context.put("items", items.getItems());
                                }
                            ...
                            context.put("itemsJson", JSONUtils.toJSONString(items.getItems()));
                        } catch (Exception e) {
                    ...
                    if (Utils.isNotEmpty(rowbtnList)) {
                        ...
                        if (tempRowItem.size() > 0) {
                            if (tempRowItem.size() > 1) {
                                context.put("itemsJson", JSONUtils.toJSONString(tempRowItem.subList(1, tempRowItem.size())));
                            } else {
                                context.put("itemsJson", JSONUtils.toJSONString(tempRowItem));
                            }
                            ...
                            context.put("rowButton0", tempRowItem.get(0));
    ...
    private void fillsearch(Map<String, Object> requestParamMap, Map<String, String> sessionParamMap, Map<String, Object> coreMap, List<Map<String, Object>> searchList, String datasourceCore) {
        ...
        for (int i = 0; i < searchList.size(); i++) {
            ...
            try {
                ...
                //如果是联合搜索，需要传入下拉搜索项
                //用于渲染页面
                context.put("itemNameList", search.get("children"));
                if (StringUtils.isNotBlank(data)) {
                    try {
                        if (StringUtils.isBlank(datasource)) {
                            datasource = datasourceCore;
                        }
                        DataSource temp = EzBootstrap.getInstance().getDataSourceByKey(datasource);
                        Map nm = new HashMap();
                        nm.putAll(requestParamMap);
                        nm.putAll(sessionParamMap);
                        ...
                        context.put("jsonArrayValue", JSONUtils.toJSONString(array));
                        context.put("itemsJson", JSONUtils.toJSONString(items.getItems()));
                    } catch (Exception e) {
...
//                        attrMap.putIfAbsent("lay-affix","clear" );
    ...
    void page(Page pagination, Map<String, Object> list, Map<String, Object> requestParamMap) throws Exception {
        ...
        context.put("encodeListId", listcode);
