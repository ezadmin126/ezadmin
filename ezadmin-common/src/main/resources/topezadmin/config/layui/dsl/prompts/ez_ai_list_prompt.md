...
| expandedWidth |
设置单元格被展开后的宽度。若设置的值的小于当前列宽，则展开后的列宽保持不变。注：当expandedMode属性为默认值时有效。 |
number | -
| expandedMode | 用于设置所有单元格默认展开方式，可选值有： | string | - tips悬浮展开方式default多行展开方式（默认）
优先级高于cellExpandedMode基础属性
