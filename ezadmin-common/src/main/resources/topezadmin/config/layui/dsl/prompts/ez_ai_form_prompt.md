...
successUrl 用于指定表单保存成功后的跳转行为，支持以下配置：

- **不配置**：默认刷新父页面
- **具体URL地址**：跳转到指定页面（支持绝对路径或相对路径）
- **reloadTableData**：仅重新加载父页面的表格数据，不刷新整个页面
- **reload**：完整刷新父页面（等同于默认行为）
- **reloadlocal**：刷新当前表单页面本身
- **refreshcard**：刷新父页面中对应iframe card的URL
...
{
  "initExpress": ["$$init.js"],
  "submitExpress": ["$$submit.js"],
  "deleteExpress": ["$$delete.js"],
