...
```json
{
  ...
  "cardList": [{
    "type": "card",
    "label": "卡片标题",
    "fieldList": [[],[]],      // 字段列表：二维数组，第一层代表行，第二层代表该行的字段
    "buttonList": []           // 按钮列表
  }],
...
#### fieldList 二维数组格式说明 - 实现复杂表单布局的核心机制

**格式规则（重要）：**
- fieldList 是**二维数组**：`[[row1_fields], [row2_fields], [row3_fields]]`
- **第一层数组**：每个元素代表表单中的**一行**（一个 div 容器）
- **第二层数组**：该行内包含的**所有字段对象**
- 每行可以包含**任意数量**的字段（1个、2个、3个、4个甚至更多）
...
```
**路径分析：**
- `/cardList/0/fieldList/0/2` - 第1行的第3个字段（索引0=STUDENT_ID, 索引1=USERNAME, 索引2=AGE）
...
2. **路径构建步骤**
   ```
   目标：修改第1个card的第1行的第2个字段（USERNAME）

   步骤1：找到cardList - /cardList
   步骤2：找到第1个card（索引0） - /cardList/0
   步骤3：找到fieldList - /cardList/0/fieldList
   步骤4：找到第1行（索引0） - /cardList/0/fieldList/0
   步骤5：从索引0开始数，包含hidden字段：
          - 索引0: STUDENT_ID (hidden) ✓
          - 索引1: USERNAME ← 这是目标！
   步骤6：完整路径 - /cardList/0/fieldList/0/1
   ```
