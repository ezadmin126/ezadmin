...
public class ControllerFilter extends Filter {
    ...
    public static String includeShow="/ezcloud/(list|form|anon)/([A-Za-z]+)-(.*)";
    Map<String, Map<String, Object>> REQUEST_MAPPING = new HashMap<String, Map<String, Object>> ();
    String pack = "com.ezcloud.controller";
    ...
    public void doFilter(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws IOException, ServletException {
       String originatingUrl =   request.getRequestURI() ;
        originatingUrl= originatingUrl.replace("/ezadmin/","/ezcloud/");
        ...
        if(m.find()&&m.groupCount()==3){
             ...
             try {
                 originatingUrl = "/ezcloud/" + contro + "/" + method+".html";
                 switch (contro) {
                     case "authc":
                     case "anon":
                     case "list":
