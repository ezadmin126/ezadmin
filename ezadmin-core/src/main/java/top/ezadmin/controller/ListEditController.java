    ...
    public void importlist(HttpServletRequest request, HttpServletResponse response) throws Exception {
        try {
            ...
            if (StringUtils.isNotBlank(listcode)) {
                response.sendRedirect(request.getContextPath() + "/topezadmin/list/loadEdit-" + listcode);
                return;
            }
            if (StringUtils.isNotBlank(formcode)) {
                response.sendRedirect(request.getContextPath() + "/topezadmin/form/loadEdit-" + formcode);
            }
        }catch (Exception e){
            e.printStackTrace();
