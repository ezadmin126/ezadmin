...
//              user.setOrgName(list.get(0).getOrgName());
//          }
//          user.setRoleNames(superMapper.selectAllRoleName(user.getUserId()));
//          user.setResourceIds(superMapper.selectAllResourceByUserId(user.getUserId()));
//          //获取所有下级
//          List<Long> childUsers=new ArrayList<>();
//            childUsers.add(user.getUserId());
//          getChildUserId(childUsers,user.getUserId(),1);
//          user.setChildUserIdList(childUsers);
...
//
//                }else{
//                    httpServletResponse.sendRedirect("/login/selectorg.html");
//                    return;
//                }
//            }else{
//                user.setOrgId(0L);
    ...
    private boolean auth(String url,HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, FilterChain filterChain) {
        try {
            ...
            for (int i = 0; i < user.getResourceIds().size(); i++) {
                ...
                if (user.getResourceIds().get(i).
                        startsWith(realUrl.replace("/ezcloud/", "/topezadmin/")
                                .replace("/ezadmin/", "/topezadmin/"))) {
                    return true;
                }
            }
            LOG.warn("auth2 null {} {}",realUrl,user.getResourceIds());
