...
public class UploadController {
    ...
    public CoreFile removeBg1(String ystype, Long fileId ,String fileType) {
        CoreFile  oriFile=coreFileMapper.selectByPrimaryKey(fileId);
        try {
            CoreFile tmFile=selectByParent(oriFile.getFileId(),fileType);
            if(tmFile!=null){
                return tmFile;
            }
            String  ossName=oriFile.getOssName();
            Date expiration = new Date(System.currentTimeMillis() + 600 * 1000); // 有效期为60秒
            com.aliyun.teaopenapi.models.Config config = new com.aliyun.teaopenapi.models.Config()
                    .setAccessKeyId(ossAccessKeyId)
                    .setAccessKeySecret(ossAccessKeySecret);
            config.endpoint = imgEndpoint;
            com.aliyun.imageseg20191230.Client client = new com.aliyun.imageseg20191230.Client(config);

            GeneratePresignedUrlRequest request = new GeneratePresignedUrlRequest(ossBucketName, ossName);
            ...
            switch (ystype) {
                case "1"://通用
                    //https://help.aliyun.com/document_detail/467470.html?spm=a2c4g.151960.0.0.183d8e7ckcARQe
                    com.aliyun.imageseg20191230.models.SegmentCommonImageRequest recognizeBankCardAdvanceRequest = new com.aliyun.imageseg20191230.models.SegmentCommonImageRequest()
                            .setImageURL(signedUrl.toString());
                    //.setReturnForm("crop")
                    SegmentCommonImageResponse resp = client.segmentCommonImage(recognizeBankCardAdvanceRequest);
                    result = resp.getBody().getData().imageURL;
                    break;
                case "2":
                    com.aliyun.imageseg20191230.models.SegmentCommodityRequest segmentCommodityRequest = new com.aliyun.imageseg20191230.models.SegmentCommodityRequest()
                            .setImageURL(signedUrl.toString());
                    //.setReturnForm("crop")
                    SegmentCommodityResponse response = client.segmentCommodityWithOptions(segmentCommodityRequest, runtime);
                    result = response.getBody().getData().imageURL;
                    break;
                default:
                    break;
            }
            InputStream in= URLUtil.getStream(new URL(result));
            tmFile= upload(oriFile.getFileName(),StringUtils.parseToByte(in),fileType);
            CoreFile upfile=new CoreFile();
            upfile.setFileId(tmFile.getFileId());
            upfile.setParentFileId(oriFile.getFileId());
            coreFileMapper.updateByPrimaryKeySelective(upfile);
            tmFile.setParentFileId(oriFile.getFileId());
            return tmFile;
        }catch (TeaException ee){
            logger.info("eeeeee{}",fileId,ee);
            return oriFile;
        }
        catch (Exception e){
            logger.error("",e);
    ...
    public  String  work(String type, Long fileId,int height) throws Exception {

//        if(height>2000||height<32){
//            return fileId+","+fileId+","+fileId ;
//        }
        StringBuilder result=new StringBuilder();
        //1抠图原图
        CoreFile c1= removeBg1(type,fileId,ORI_TM_FILE);
        if(c1==null){
            result.append(fileId+",");
        }else{
            result.append(c1.getFileId()+",");
        }
        //2压缩
        CoreFile c2 = thumbnailFile(fileId,height);
        result.append(c2.getFileId()+",");
        //3压缩抠图
        CoreFile c3= removeBg1(type,c2.getFileId(),ORI_YS_TM_TYPE);
        if(c3==null){
            result.append(fileId+",");
        }else{
            result.append(c3.getFileId()+",");
        }
